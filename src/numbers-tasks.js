...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const modulFirstVector = Math.sqrt(x1 * x1 + y1 * y1);
  const modulSecondVector = Math.sqrt(x2 * x2 + y2 * y2);
  const cosinus = multOfVectors / (modulFirstVector * modulSecondVector);
...
function roundToPowerOfTen(num, pow) {
  const spec = 10 ** pow;
  return Math.round(Math.floor(num) / spec) * spec;
}
